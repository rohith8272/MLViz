...
(function (OrbitYawBehaviour) {
    ...
    /**
     * Vehicle uses current yaw behaviour (unchanged). The vehicle-default yaw behaviour is used if this
     * value is specified when orbit is first commanded.
     */
    OrbitYawBehaviour[OrbitYawBehaviour["UNCHANGED"] = 5] = "UNCHANGED";
...
/**
 * Axes that will be autotuned by MAV_CMD_DO_AUTOTUNE_ENABLE. Note that at least one flag must be set
 * in MAV_CMD_DO_AUTOTUNE_ENABLE.param2: if none are set, the flight stack will tune its default set of
 * axes.
 */
var AutotuneAxis;
 ...
 */
var RebootShutdownAction;
(function (RebootShutdownAction) {
    /**
     * Do nothing.
     */
    RebootShutdownAction[RebootShutdownAction["NONE"] = 0] = "NONE";
    /**
     * Reboot component.
     */
    RebootShutdownAction[RebootShutdownAction["REBOOT"] = 1] = "REBOOT";
    /**
     * Shutdown component.
     */
    RebootShutdownAction[RebootShutdownAction["SHUTDOWN"] = 2] = "SHUTDOWN";
    /**
     * Reboot component and keep it in the bootloader until upgraded.
     */
    RebootShutdownAction[RebootShutdownAction["REBOOT_TO_BOOTLOADER"] = 3] = "REBOOT_TO_BOOTLOADER";
    /**
     * Power on component. Do nothing if component is already powered (ACK command with
     * MAV_RESULT_ACCEPTED).
     */
    RebootShutdownAction[RebootShutdownAction["POWER_ON"] = 4] = "POWER_ON";
})(RebootShutdownAction = exports.RebootShutdownAction || (exports.RebootShutdownAction = {}));
/**
 * Specifies the conditions under which the MAV_CMD_PREFLIGHT_REBOOT_SHUTDOWN command should be
 * accepted.
 */
var RebootShutdownConditions;
(function (RebootShutdownConditions) {
    /**
     * Reboot/Shutdown only if allowed by safety checks, such as being landed.
     */
    RebootShutdownConditions[RebootShutdownConditions["SAFETY_INTERLOCKED"] = 0] = "SAFETY_INTERLOCKED";
    /**
     * Force reboot/shutdown of the autopilot/component regardless of system state.
     */
    RebootShutdownConditions[RebootShutdownConditions["FORCE"] = 20190226] = "FORCE";
})(RebootShutdownConditions = exports.RebootShutdownConditions || (exports.RebootShutdownConditions = {}));
/**
...
(function (MavCmd) {
     ...
     * @param2 Heading Required Leave loiter circle only when track heads towards the next waypoint (MAV_BOOL_FALSE: Leave when turns complete). Values not equal to 0 or 1 are invalid.
     * @param3 Radius[m] Loiter radius around waypoint for forward-only moving vehicles (not multicopters). If positive loiter clockwise, else counter-clockwise
     ...
     * @param2 Heading Required Leave loiter circle only when track heading towards the next waypoint (MAV_BOOL_FALSE: Leave on time expiry). Values not equal to 0 or 1 are invalid.
     * @param3 Radius[m] Loiter radius around waypoint for forward-only moving vehicles (not multicopters). If positive loiter clockwise, else counter-clockwise.
     * @param4 Xtrack Location Loiter circle exit location and/or path to next waypoint ("xtrack") for forward-only moving vehicles (not multicopters). 0 for the vehicle to converge towards the center xtrack when it leaves the loiter (the line between the centers of the current and next waypoint), 1 to converge to the direct line between the location that the vehicle exits the loiter radius and the next waypoint. Otherwise the angle (in degrees) between the tangent of the loiter circle and the center xtrack at which the vehicle must leave the loiter (and converge to the center xtrack). NaN to use the current system default xtrack behaviour.
     * @param5 Latitude Latitude
     * @param6 Longitude Longitude
     ...
     * @note has location and is destination
     *
     * @param1 Heading Required Leave loiter circle only when track heading towards the next waypoint (MAV_BOOL_FALSE: Leave when altitude reached). Values not equal to 0 or 1 are invalid.
     ...
     * @param1 Enable Guided mode on (MAV_BOOL_FALSE: Off). Values not equal to 0 or 1 are invalid.
     * @param2 Empty
     * @param3 Empty
     * @param4 Empty
     * @param5 Empty
     ...
     * @param4 Relative Relative offset (MAV_BOOL_FALSE: absolute angle). Values not equal to 0 or 1 are invalid.
     * @param5 Empty
     * @param6 Empty
     * @param7 Empty
     */
    ...
    /**
     * Set system mode.
     * @param1 Mode Mode flags. MAV_MODE values can be used to set some mode flag combinations.
     * @param2 Custom Mode Custom system-specific mode (see target autopilot specifications for mode information). If MAV_MODE_FLAG_CUSTOM_MODE_ENABLED is set in param1 (mode) this mode is used: otherwise the field is ignored.
     ...
     * @note has location
     *
     * @param1 Use Current Use current location (MAV_BOOL_FALSE: use specified location). Values not equal to 0 or 1 are invalid.
     * @param2 Roll[deg] (min: -180, max: 180) Roll angle (of surface). Range: -180..180 degrees. NAN or 0 means value not set. 0.01 indicates zero roll.
     * @param3 Pitch[deg] (min: -90, max: 90) Pitch angle (of surface). Range: -90..90 degrees. NAN or 0 means value not set. 0.01 means zero pitch.
     * @param4 Yaw[deg] (min: -180, max: 180) Yaw angle. NaN to use default heading. Range: -180..180 degrees.
     * @param5 Latitude Latitude
     * @param6 Longitude Longitude
     * @param7 Altitude[m] Altitude
     */
    ...
    /**
     * Terminate flight immediately. Flight termination immediately and irreversibly terminates the current
    ...
    /**
     * Mission item to specify the start of a failsafe/landing return-path segment (the end of the segment
     * is the next MAV_CMD_DO_LAND_START item). A vehicle that is using missions for landing (e.g. in a
     * return mode) will join the mission on the closest path of the return-path segment (instead of
     * MAV_CMD_DO_LAND_START or the nearest waypoint). The main use case is to minimize the failsafe flight
     * path in corridor missions, where the inbound/outbound paths are constrained (by geofences) to the
     * same particular path. The MAV_CMD_NAV_RETURN_PATH_START would be placed at the start of the return
     * path. If a failsafe occurs on the outbound path the vehicle will move to the nearest point on the
     * return path (which is parallel for this kind of mission), effectively turning round and following
     * the shortest path to landing. If a failsafe occurs on the inbound path the vehicle is already on the
     * return segment and will continue to landing. The Latitude/Longitude/Altitude are optional, and may
     * be set to 0 if not needed. If specified, the item defines the waypoint at which the return segment
     * starts. If sent using as a command, the vehicle will perform a mission landing (using the land
     * segment if defined) or reject the command if mission landings are not supported, or no mission
     * landing is defined. When used as a command any position information in the command is ignored.
     *
     * @note has location
     *
     * @param1 Empty
     ...
     * @param3 Empty
     * @param4 Empty
     * @param5 Latitude Latitudee. 0: not used.
     * @param6 Longitude Longitudee. 0: not used.
     * @param7 Altitude[m] Altitudee. 0: not used.
     */
    MavCmd[MavCmd["DO_RETURN_PATH_START"] = 188] = "DO_RETURN_PATH_START";
    /**
     * Mission item to mark the start of a mission landing pattern, or a command to land with a mission
     * landing pattern. When used in a mission, this is a marker for the start of a sequence of mission
     * items that represent a landing pattern. It should be followed by a navigation item that defines the
     * first waypoint of the landing sequence. The start marker positional params are used only for
     * selecting what landing pattern to use if several are defined in the mission (the selected pattern
     * will be the one with the marker position that is closest to the vehicle when a landing is
     * commanded). If the marker item position has zero-values for latitude, longitude, and altitude, then
     * landing pattern selection is instead based on the position of the first waypoint in the landing
     * sequence. 	 When sent as a command it triggers a landing using a mission landing pattern. 	 The
     * location parameters are not used in this case, and should be set to 0.
     *
     * @note has location
     *
     * @param1 Empty
     ...
     * @param3 Empty
     * @param4 Empty
     * @param5 Latitude Latitude for landing sequence selection, or 0 (see description). Ignored in commands (set 0).
     * @param6 Longitude Longitude for landing sequence selection, or 0 (see description). Ignored in commands (set 0).
     * @param7 Altitude[m] Altitude for landing sequence selection, or 0 (see description). Ignored in commands (set 0).
     */
     ...
     * @param4 Yaw[rad] Yaw heading (heading reference defined in Bitmask field). NaN to use the current system yaw heading mode (e.g. yaw towards next waypoint, yaw to home, etc.). For planes indicates loiter direction (0: clockwise, 1: counter clockwise)
     * @param5 Latitude Latitude
     * @param6 Longitude Longitude
     * @param7 Altitude[m] Altitude
     */
     ...
     * @param1 Continue Continue mission (MAV_BOOL_TRUE), Pause current mission or reposition command, hold current position (MAV_BOOL_FALSE). Values not equal to 0 or 1 are invalid. A VTOL capable vehicle should enter hover mode (multicopter and VTOL planes). A plane should loiter with the default loiter radius.
     * @param2 Reserved
     * @param3 Reserved
     * @param4 Reserved
     * @param5 Reserved
     ...
     * @param1 Reverse Reverse direction (MAV_BOOL_FALSE: Forward direction). Values not equal to 0 or 1 are invalid.
     * @param2 Empty
     * @param3 Empty
     * @param4 Empty
     * @param5 Empty
     ...
     * @param3 Empty
     * @param4 Empty
     * @param5 Pitch Offset[deg] Pitch offset from next waypoint, positive pitching up
     * @param6 Roll Offset[deg] Roll offset from next waypoint, positive rolling to the right
     * @param7 Yaw Offset[deg] Yaw offset from next waypoint, positive yawing to the right
     */
     ...
     * @param2 Stabilize Roll Stabilize roll (MAV_BOOL_TRUE). Values not equal to 0 or 1 are invalid.
     * @param3 Stabilize Pitch Stabilize pitch (MAV_BOOL_TRUE). Values not equal to 0 or 1 are invalid.
     * @param4 Stabilize Yaw Stabilize yaw (MAV_BOOL_TRUE). Values not equal to 0 or 1 are invalid.
     ...
     * @param3 Trigger Trigger camera once, immediately (MAV_BOOL_TRUE). Values not equal to 0 or 1 are invalid.
     * @param4 Target Camera ID (min: 0, max: 255, increment: 1) Target camera ID. 7 to 255: MAVLink camera component id. 1 to 6 for cameras attached to the autopilot, which don't have a distinct component id. 0: all cameras. This is used to target specific autopilot-connected cameras. It is also used to target specific cameras when the MAV_CMD is used in a mission.
     * @param5 Empty
     * @param6 Empty
     * @param7 Empty
     */
    ...
    /**
     * Enable the geofence. This can be used in a mission or via the command protocol. The
     * persistence/lifetime of the setting is undefined. Depending on flight stack implementation it may
     * persist until superseded, or it may revert to a system default at the end of a mission. Flight
     * stacks typically reset the setting to system defaults on reboot.
     * @param1 Enable (min: 0, max: 2, increment: 1) enable? (0=disable, 1=enable, 2=disable_floor_only)
     * @param2 Types Fence types to enable or disable as a bitmask. 0: field is unused/all fences should be enabled or disabled (for compatibility reasons). Parameter is ignored if param1=2.
     * @param3 Empty
     * @param4 Empty
     * @param5 Empty
     * @param6 Empty
     ...
     * @param1 Inverted Inverted flight (MAV_BOOL_False: normal flight). Values not equal to 0 or 1 are invalid.
     * @param2 Empty
     * @param3 Empty
     * @param4 Empty
     * @param5 Empty
     ...
     * @param1 Gripper ID (min: 1, increment: 1) Gripper ID. 1-6 for an autopilot connected gripper. In missions this may be set to 1-6 for an autopilot gripper, or the gripper component id for a MAVLink gripper. 0 targets all grippers.
     ...
     * @param1 Enable Enable autotune (MAV_BOOL_FALSE: disable autotune). Values not equal to 0 or 1 are invalid.
     * @param2 Axis Specify axes for which autotuning is enabled/disabled. 0 indicates the field is unused (for compatibility reasons). If 0 the autopilot will follow its default behaviour, which is usually to tune all axes.
     * @param3 Empty.
     * @param4 Empty.
     * @param5 Empty.
     * @param6 Empty.
     ...
     * @param3 Angle Relative final angle (MAV_BOOL_FALSE: Absolute angle). Values not equal to 0 or 1 are invalid.
     * @param4 Empty
     * @param5 Empty
     * @param6 Empty
     * @param7 Empty
     ...
     * @param3 Target Camera ID (min: 0, max: 255, increment: 1) Target camera ID. 7 to 255: MAVLink camera component id. 1 to 6 for cameras attached to the autopilot, which don't have a distinct component id. 0: all cameras. This is used to target specific autopilot-connected cameras. It is also used to target specific cameras when the MAV_CMD is used in a mission.
     * @param4 Empty
     * @param5 Empty
     * @param6 Empty
     * @param7 Empty
     ...
     * @param1 Start Engine Start engine (MAV_BOOL_False: Stop engine). Values not equal to 0 or 1 are invalid.
     * @param2 Cold Start Cold start engine (MAV_BOOL_FALSE: Warm start). Values not equal to 0 or 1 are invalid. Controls use of choke where applicable
     ...
     * @param2 Reset Mission Reset mission (MAV_BOOL_TRUE). Values not equal to 0 or 1 are invalid. Resets jump counters to initial values and changes mission state "completed" to be "active" or "paused".
     * @param3 Empty
     * @param4 Empty
     * @param5 Empty
     * @param6 Empty
     ...
     * @param2 Magnetometer Magnetometer calibration. Values not equal to 0 or 1 are invalid.
     * @param3 Ground Pressure Ground pressure calibration. Values not equal to 0 or 1 are invalid.
     ...
     * @param1 Autopilot Action to take for autopilot.
     * @param2 Companion Action to take for onboard computer.
     * @param3 Component Action Action to take for component specified in param4.
     ...
     * @param6 Conditions Conditions under which reboot/shutdown is allowed.
    ...
    /**
     * Enable the specified standard MAVLink mode. If the specified mode is not supported, the vehicle
     * should ACK with MAV_RESULT_FAILED. See https://mavlink.io/en/services/standard_modes.html
     ...
     * on. Autopilots must NACK this command with MAV_RESULT_TEMPORARILY_REJECTED while armed.
     ...
     * @param1 Arm Arm (MAV_BOOL_FALSE: disarm). Values not equal to 0 or 1 are invalid.
     ...
     * external to the system: e.g. a torch or searchlight (as opposed to a light source for illuminating
     * the system itself, e.g. an indicator light).
     * @param1 Enable Illuminators on/off (MAV_BOOL_TRUE: illuminators on). Values not equal to 0 or 1 are invalid.
     */
    MavCmd[MavCmd["ILLUMINATOR_ON_OFF"] = 405] = "ILLUMINATOR_ON_OFF";
    /**
     * Configures illuminator settings. An illuminator is a light source that is used for lighting up dark
     ...
     * @param1 Mode Mode
     * @param2 Brightness[%] (min: 0, max: 100) 0%: Off, 100%: Max Brightness
     * @param3 Strobe Period[s] (min: 0) Strobe period in seconds where 0 means strobing is not used
     * @param4 Strobe Duty[%] (min: 0, max: 100) Strobe duty cycle where 100% means it is on constantly and 0 means strobing is not used
     */
    MavCmd[MavCmd["DO_ILLUMINATOR_CONFIGURE"] = 406] = "DO_ILLUMINATOR_CONFIGURE";
    /**
     ...
     * @param1 RC Type RC type.
     * @param2 RC Sub Type RC sub type.
     */
     ...
     * @param3 Req Param 3 Use for index ID, if required. Otherwise, the use of this parameter (if any) must be defined in the requested message. By default assumed not used (0).  When used as an index ID, 0 means "all instances", "1" means the first instance in the sequence (the emitted message will have an id of 0 if message ids are 0-indexed, or 1 if index numbers start from one).
     * @param4 Req Param 4 The use of this parameter (if any), must be defined in the requested message. By default assumed not used (0).
     * @param5 Req Param 5 The use of this parameter (if any), must be defined in the requested message. By default assumed not used (0/NaN).
     * @param6 Req Param 6 The use of this parameter (if any), must be defined in the requested message. By default assumed not used (0/NaN).
     * @param7 Response Target (min: 0, max: 2, increment: 1) Target address of message stream (if message has target address fields). 0: Flight-stack default (recommended), 1: address of requester, 2: broadcast.
     */
     ...
     * @param7 Response Target (min: 0, max: 2, increment: 1) Target address for requested message (if message has target address fields). 0: Flight-stack default, 1: address of requester, 2: broadcast.
     */
     ...
     * @param1 Protocol Request supported protocol versions by all nodes on the network (MAV_BOOL_TRUE). Values not equal to 0 or 1 are invalid.
     ...
     * @param1 Version Request autopilot version (MAV_BOOL_TRUE). Values not equal to 0 or 1 are invalid.
     * @param2 Reserved (all remaining params)
     */
    MavCmd[MavCmd["REQUEST_AUTOPILOT_CAPABILITIES"] = 520] = "REQUEST_AUTOPILOT_CAPABILITIES";
    /**
     * Request camera information (CAMERA_INFORMATION).
     * @param1 Capabilities Request camera capabilities (MAV_BOOL_TRUE). Values not equal to 0 or 1 are invalid.
     * @param2 Reserved (all remaining params)
     */
    MavCmd[MavCmd["REQUEST_CAMERA_INFORMATION"] = 521] = "REQUEST_CAMERA_INFORMATION";
    /**
     * Request camera settings (CAMERA_SETTINGS).
     * @param1 Settings Request camera settings (MAV_BOOL_TRUE). Values not equal to 0 or 1 are invalid.
     * @param2 Reserved (all remaining params)
     */
     ...
     * @param2 Information Request storage information (MAV_BOOL_TRUE). Values not equal to 0 or 1 are invalid.
     ...
     * @param2 Format Format storage (and reset image log). Values not equal to 0 or 1 are invalid.
     * @param3 Reset Image Log Reset Image Log (without formatting storage medium). This will reset CAMERA_CAPTURE_STATUS.image_count and CAMERA_IMAGE_CAPTURED.image_index. Values not equal to 0 or 1 are invalid.
     * @param4 Reserved (all remaining params)
     */
    MavCmd[MavCmd["STORAGE_FORMAT"] = 526] = "STORAGE_FORMAT";
    /**
     * Request camera capture status (CAMERA_CAPTURE_STATUS)
     * @param1 Capture Status Request camera capture status (MAV_BOOL_TRUE). Values not equal to 0 or 1 are invalid.
     * @param2 Reserved (all remaining params)
     */
    MavCmd[MavCmd["REQUEST_CAMERA_CAPTURE_STATUS"] = 527] = "REQUEST_CAMERA_CAPTURE_STATUS";
    /**
     * Request flight information (FLIGHT_INFORMATION)
     * @param1 Flight Information Request flight information (MAV_BOOL_TRUE). Values not equal to 0 or 1 are invalid.
     * @param2 Reserved (all remaining params)
     */
    MavCmd[MavCmd["REQUEST_FLIGHT_INFORMATION"] = 528] = "REQUEST_FLIGHT_INFORMATION";
    /**
     * Reset all camera settings to Factory Default
     * @param1 Reset Reset all settings (MAV_BOOL_TRUE). Values not equal to 0 or 1 are invalid.
     * @param2 Target Camera ID (min: 0, max: 255, increment: 1) Target camera ID. 7 to 255: MAVLink camera component id. 1 to 6 for cameras attached to the autopilot, which don't have a distinct component id. 0: all cameras. This is used to target specific autopilot-connected cameras. It is also used to target specific cameras when the MAV_CMD is used in a mission.
     */
     ...
     * streaming.
     * @param1 id (min: 0, max: 255, increment: 1) Target camera ID. 7 to 255: MAVLink camera component id. 1 to 6 for cameras attached to the autopilot, which don't have a distinct component id. 0: all cameras. This is used to target specific autopilot-connected cameras. It is also used to target specific cameras when the MAV_CMD is used in a mission.
     ...
     */
    MavCmd[MavCmd["SET_CAMERA_SOURCE"] = 534] = "SET_CAMERA_SOURCE";
    /**
     ...
     * @param3 Radius (min: 0, max: 1) Point radius (normalized 0..1, 0 is one pixel, 1 is full image width).
     * @param4 Target Camera ID (min: 0, max: 255, increment: 1) Target camera ID. 7 to 255: MAVLink camera component id. 1 to 6 for cameras attached to the autopilot, which don't have a distinct component id. 0: all cameras. This is used to target specific autopilot-connected cameras. It is also used to target specific cameras when the MAV_CMD is used in a mission.
     */
     ...
     * @param5 Target Camera ID (min: 0, max: 255, increment: 1) Target camera ID. 7 to 255: MAVLink camera component id. 1 to 6 for cameras attached to the autopilot, which don't have a distinct component id. 0: all cameras. This is used to target specific autopilot-connected cameras. It is also used to target specific cameras when the MAV_CMD is used in a mission.
     */
     ...
     * @param1 Enable Start transmission over high latency telemetry (MAV_BOOL_FALSE: stop transmission). Values not equal to 0 or 1 are invalid.
     * @param2 Empty
     * @param3 Empty
     * @param4 Empty
     * @param5 Empty
     ...
     * the COMMAND_ACK message progress field should be set with period of time that this authorization is
     * valid in seconds. 		If the authorization is denied COMMAND_ACK.result_param2 should be set with one
     * of the reasons in ARM_AUTH_DENIED_REASON.
     ...
     * @param2 UseAltitude Use altitude (MAV_BOOL_FALSE: ignore altitude). Values not equal to 0 or 1 are invalid.
     * @param3 Empty
     * @param4 Empty
     * @param5 Latitude Latitude
     * @param6 Longitude Longitude
     ...
     * stay within this area. Minimum of 3 vertices required. The vertices for a polygon must be sent
     * sequentially, each with param1 set to the total number of vertices in the polygon.
     *
     * @note has location
     *
     * @param1 Vertex Count (min: 3, increment: 1) Polygon vertex count. This is the number of vertices in the current polygon (all vertices will have the same number).
     ...
     * stay outside this area. Minimum of 3 vertices required. The vertices for a polygon must be sent
     * sequentially, each with param1 set to the total number of vertices in the polygon.
     *
     * @note has location
     *
     ...
     * @param5 Latitude[degE7] Latitude.
     * @param6 Longitude[degE7] Longitude.
...
(function (MavResult) {
     ...
     */
    MavResult[MavResult["COMMAND_UNSUPPORTED_MAV_FRAME"] = 9] = "COMMAND_UNSUPPORTED_MAV_FRAME";
...
(function (MavDoRepositionFlags) {
     ...
     */
    MavDoRepositionFlags[MavDoRepositionFlags["RELATIVE_YAW"] = 2] = "RELATIVE_YAW";
...
(function (CameraCapFlags) {
     ...
     */
    CameraCapFlags[CameraCapFlags["HAS_THERMAL_RANGE"] = 4096] = "HAS_THERMAL_RANGE";
    /**
     * Camera supports Moving Target Indicators (MTI) on the camera view (using MAV_CMD_CAMERA_START_MTI).
     */
    CameraCapFlags[CameraCapFlags["HAS_MTI"] = 8192] = "HAS_MTI";
...
(function (VideoStreamStatusFlags) {
     ...
     */
    VideoStreamStatusFlags[VideoStreamStatusFlags["THERMAL_RANGE_ENABLED"] = 4] = "THERMAL_RANGE_ENABLED";
...
(function (VideoStreamType) {
     ...
     */
    VideoStreamType[VideoStreamType["MPEG_TS"] = 3] = "MPEG_TS";
})(VideoStreamType = exports.VideoStreamType || (exports.VideoStreamType = {}));
/**
 * Video stream encodings
 */
var VideoStreamEncoding;
(function (VideoStreamEncoding) {
    /**
     * Stream encoding is unknown
     */
    VideoStreamEncoding[VideoStreamEncoding["UNKNOWN"] = 0] = "UNKNOWN";
    /**
     * Stream encoding is H.264
     */
    VideoStreamEncoding[VideoStreamEncoding["H264"] = 1] = "H264";
    /**
     * Stream encoding is H.265
     */
    VideoStreamEncoding[VideoStreamEncoding["H265"] = 2] = "H265";
})(VideoStreamEncoding = exports.VideoStreamEncoding || (exports.VideoStreamEncoding = {}));
/**
...
(function (CameraTrackingStatusFlags) {
     ...
     */
    CameraTrackingStatusFlags[CameraTrackingStatusFlags["MTI"] = 4] = "MTI";
    /**
     * Camera tracking target is obscured and is being predicted
     */
    CameraTrackingStatusFlags[CameraTrackingStatusFlags["COASTING"] = 8] = "COASTING";
...
(function (CameraZoomType) {
     ...
     */
    CameraZoomType[CameraZoomType["HORIZONTAL_FOV"] = 4] = "HORIZONTAL_FOV";
...
(function (CameraSource) {
     ...
     */
    CameraSource[CameraSource["DEFAULT"] = 0] = "DEFAULT";
    /**
     * RGB camera source.
     */
    CameraSource[CameraSource["RGB"] = 1] = "RGB";
    /**
     * IR camera source.
     */
    CameraSource[CameraSource["IR"] = 2] = "IR";
    /**
     * NDVI camera source.
     */
    CameraSource[CameraSource["NDVI"] = 3] = "NDVI";
})(CameraSource = exports.CameraSource || (exports.CameraSource = {}));
/**
...
(function (ParamAck) {
    ...
    /**
     * Parameter value received but not yet set/accepted. A subsequent PARAM_EXT_ACK with the final result
     * will follow once operation is completed. This is returned immediately for parameters that take
     * longer to set, indicating that the the parameter was received and does not need to be resent.
     */
...
(function (RcType) {
     ...
     * Spektrum
     */
    RcType[RcType["SPEKTRUM"] = 0] = "SPEKTRUM";
    /**
     * CRSF
     */
    RcType[RcType["CRSF"] = 1] = "CRSF";
...
(function (RcSubType) {
     ...
     * Spektrum DSM2
     */
    RcSubType[RcSubType["SPEKTRUM_DSM2"] = 0] = "SPEKTRUM_DSM2";
    /**
     * Spektrum DSMX
     */
    RcSubType[RcSubType["SPEKTRUM_DSMX"] = 1] = "SPEKTRUM_DSMX";
    /**
     * Spektrum DSMX8
     */
    RcSubType[RcSubType["SPEKTRUM_DSMX8"] = 2] = "SPEKTRUM_DSMX8";
})(RcSubType = exports.RcSubType || (exports.RcSubType = {}));
/**
...
(function (MavTunnelPayloadType) {
     ...
     */
    MavTunnelPayloadType[MavTunnelPayloadType["MODALAI_REMOTE_OSD"] = 210] = "MODALAI_REMOTE_OSD";
    /**
     * Registered for ModalAI ESC UART passthru protocol.
     */
    MavTunnelPayloadType[MavTunnelPayloadType["MODALAI_ESC_UART_PASSTHRU"] = 211] = "MODALAI_ESC_UART_PASSTHRU";
    /**
     * Registered for ModalAI vendor use.
     */
    MavTunnelPayloadType[MavTunnelPayloadType["MODALAI_IO_UART_PASSTHRU"] = 212] = "MODALAI_IO_UART_PASSTHRU";
 ...
 */
var IlluminatorMode;
(function (IlluminatorMode) {
    /**
     * Illuminator mode is not specified/unknown
     */
    IlluminatorMode[IlluminatorMode["UNKNOWN"] = 0] = "UNKNOWN";
    /**
     * Illuminator behavior is controlled by MAV_CMD_DO_ILLUMINATOR_CONFIGURE settings
     */
    IlluminatorMode[IlluminatorMode["INTERNAL_CONTROL"] = 1] = "INTERNAL_CONTROL";
    /**
     * Illuminator behavior is controlled by external factors: e.g. an external hardware signal
     */
    IlluminatorMode[IlluminatorMode["EXTERNAL_SYNC"] = 2] = "EXTERNAL_SYNC";
})(IlluminatorMode = exports.IlluminatorMode || (exports.IlluminatorMode = {}));
/**
 * Illuminator module error flags (bitmap, 0 means no error)
 */
var IlluminatorErrorFlags;
(function (IlluminatorErrorFlags) {
    /**
     * Illuminator thermal throttling error.
     */
    IlluminatorErrorFlags[IlluminatorErrorFlags["THERMAL_THROTTLING"] = 1] = "THERMAL_THROTTLING";
    /**
     * Illuminator over temperature shutdown error.
     */
    IlluminatorErrorFlags[IlluminatorErrorFlags["OVER_TEMPERATURE_SHUTDOWN"] = 2] = "OVER_TEMPERATURE_SHUTDOWN";
    /**
     * Illuminator thermistor failure.
     */
    IlluminatorErrorFlags[IlluminatorErrorFlags["THERMISTOR_FAILURE"] = 4] = "THERMISTOR_FAILURE";
})(IlluminatorErrorFlags = exports.IlluminatorErrorFlags || (exports.IlluminatorErrorFlags = {}));
/**
 ...
 * though the precise mechanics of this mode may differ. The modes supported by a flight stack can be
 * queried using AVAILABLE_MODES and set using MAV_CMD_DO_SET_STANDARD_MODE. The current mode is
 * streamed in CURRENT_MODE. See https://mavlink.io/en/services/standard_modes.html
 */
var MavStandardMode;
(function (MavStandardMode) {
     ...
     * safe flight path, and may also automatically land the vehicle. This mode is more commonly referred
     * to as RTL and/or or Smart RTL. The precise return location, flight path, and landing behaviour
     * depend on vehicle configuration and type. For example, the vehicle might return to the home/launch
     * location, a rally point, or the start of a mission landing, it might follow a direct path, mission
     * path, or breadcrumb path, and land using a mission landing pattern or some other kind of descent.
     */
    MavStandardMode[MavStandardMode["SAFE_RECOVERY"] = 5] = "SAFE_RECOVERY";
    /**
     ...
     */
    MavStandardMode[MavStandardMode["MISSION"] = 6] = "MISSION";
    /**
...
(function (MavModeProperty) {
     ...
     */
    MavModeProperty[MavModeProperty["AUTO_MODE"] = 4] = "AUTO_MODE";
})(MavModeProperty = exports.MavModeProperty || (exports.MavModeProperty = {}));
/**
 * Flags used in HIL_ACTUATOR_CONTROLS message.
 */
var HilActuatorControlsFlags;
(function (HilActuatorControlsFlags) {
    /**
     * Simulation is using lockstep
     */
    HilActuatorControlsFlags[HilActuatorControlsFlags["LOCKSTEP"] = 1] = "LOCKSTEP";
})(HilActuatorControlsFlags = exports.HilActuatorControlsFlags || (exports.HilActuatorControlsFlags = {}));
/**
...
/**
 * The system time is the time of the master clock. This can be emitted by flight controllers, onboard
 * computers, or other components in the MAVLink network. Components that are using a less reliable
 * time source, such as a battery-backed real time clock, can choose to match their system clock to
 * that of a SYSTEM_TYPE that indicates a more recent time. This allows more broadly accurate date
 * stamping of logs, and so on. If precise time synchronization is needed then use TIMESYNC instead.
 */
 ...
 *
 * @deprecated since 2011-08, replaced by TIMESYNC; To be removed / merged with TIMESYNC
 */
...
GpsRawInt.FIELDS = [
    ...
    new mavlink_1.MavLinkPacketField('vel_acc', 'velAcc', 42, true, 4, 'uint32_t', 'mm/s'),
 ...
 * are inactive should be set to INT16_MAX.
 */
...
MissionCurrent.FIELDS = [
    ...
    new mavlink_1.MavLinkPacketField('mission_id', 'missionId', 6, true, 4, 'uint32_t', ''),
    new mavlink_1.MavLinkPacketField('fence_id', 'fenceId', 10, true, 4, 'uint32_t', ''),
    new mavlink_1.MavLinkPacketField('rally_points_id', 'rallyPointsId', 14, true, 4, 'uint32_t', ''),
];
/**
...
MissionCount.FIELDS = [
    ...
    new mavlink_1.MavLinkPacketField('target_component', 'targetComponent', 3, false, 1, 'uint8_t', ''),
    new mavlink_1.MavLinkPacketField('mission_type', 'missionType', 4, true, 1, 'uint8_t', ''),
    new mavlink_1.MavLinkPacketField('opaque_id', 'opaqueId', 5, true, 4, 'uint32_t', ''),
];
/**
...
MissionAck.FIELDS = [
    ...
    new mavlink_1.MavLinkPacketField('opaque_id', 'opaqueId', 4, true, 4, 'uint32_t', ''),
];
/**
...
class HilActuatorControls extends mavlink_1.MavLinkData {
    constructor() {
        ...
        this.flags = HilActuatorControlsFlags[Object.keys(HilActuatorControlsFlags)[0]];
    }
}
 ...
 * and cannot reliably timesync; the requester may report an error. Timestamps are UNIX Epoch time or
 ...
 * sequence is repeated numerous times with results being filtered/averaged to estimate the offset. See
 * also: https://mavlink.io/en/services/timesync.html.
 */
...
Gps2Raw.FIELDS = [
    ...
    new mavlink_1.MavLinkPacketField('vel_acc', 'velAcc', 49, true, 4, 'uint32_t', 'mm/s'),
...
class LandingTarget extends mavlink_1.MavLinkData {
    constructor() {
        ...
        this.positionValid = standard_1.MavBool[Object.keys(standard_1.MavBool)[0]];
    }
}
...
CameraInformation.PAYLOAD_LENGTH = 237;
...
CameraInformation.FIELDS = [
    ...
    new mavlink_1.MavLinkPacketField('camera_device_id', 'cameraDeviceId', 236, true, 1, 'uint8_t', ''),
];
/**
...
CameraSettings.PAYLOAD_LENGTH = 14;
...
CameraSettings.FIELDS = [
    ...
    new mavlink_1.MavLinkPacketField('camera_device_id', 'cameraDeviceId', 13, true, 1, 'uint8_t', ''),
];
/**
...
CameraCaptureStatus.PAYLOAD_LENGTH = 23;
...
CameraCaptureStatus.FIELDS = [
    ...
    new mavlink_1.MavLinkPacketField('camera_device_id', 'cameraDeviceId', 22, true, 1, 'uint8_t', ''),
];
/**
...
class CameraImageCaptured extends mavlink_1.MavLinkData {
    constructor() {
        ...
        this.q = [];
        this.imageIndex = 0;
        this.captureResult = standard_1.MavBool[Object.keys(standard_1.MavBool)[0]];
        this.fileUrl = '';
    }
}
...
/**
 * Flight information. This includes time since boot for arm, takeoff, and land, and a flight number.
 * Takeoff and landing values reset to zero on arm. This can be requested using
 * MAV_CMD_REQUEST_MESSAGE. Note, some fields are misnamed - timestamps are from boot (not UTC) and the
...
FlightInformation.PAYLOAD_LENGTH = 32;
...
FlightInformation.FIELDS = [
    ...
    new mavlink_1.MavLinkPacketField('landing_time', 'landingTime', 28, true, 4, 'uint32_t', 'ms'),
];
/**
...
class VideoStreamInformation extends mavlink_1.MavLinkData {
    constructor() {
        ...
        this.name = '';
        this.uri = '';
        this.encoding = VideoStreamEncoding[Object.keys(VideoStreamEncoding)[0]];
        this.cameraDeviceId = 0;
    }
}
...
VideoStreamInformation.PAYLOAD_LENGTH = 215;
...
VideoStreamInformation.FIELDS = [
    ...
    new mavlink_1.MavLinkPacketField('encoding', 'encoding', 213, true, 1, 'uint8_t', ''),
    new mavlink_1.MavLinkPacketField('camera_device_id', 'cameraDeviceId', 214, true, 1, 'uint8_t', ''),
];
/**
...
VideoStreamStatus.PAYLOAD_LENGTH = 20;
...
VideoStreamStatus.FIELDS = [
    ...
    new mavlink_1.MavLinkPacketField('hfov', 'hfov', 16, false, 2, 'uint16_t', 'deg'),
    new mavlink_1.MavLinkPacketField('stream_id', 'streamId', 18, false, 1, 'uint8_t', ''),
    new mavlink_1.MavLinkPacketField('camera_device_id', 'cameraDeviceId', 19, true, 1, 'uint8_t', ''),
];
/**
...
CameraFovStatus.PAYLOAD_LENGTH = 53;
...
CameraFovStatus.FIELDS = [
    ...
    new mavlink_1.MavLinkPacketField('camera_device_id', 'cameraDeviceId', 52, true, 1, 'uint8_t', ''),
];
/**
...
CameraTrackingImageStatus.PAYLOAD_LENGTH = 32;
...
CameraTrackingImageStatus.FIELDS = [
    ...
    new mavlink_1.MavLinkPacketField('camera_device_id', 'cameraDeviceId', 31, true, 1, 'uint8_t', ''),
];
/**
 * Camera tracking status, sent while in active tracking. Use MAV_CMD_SET_MESSAGE_INTERVAL to define
 * message interval.
...
CameraTrackingGeoStatus.PAYLOAD_LENGTH = 50;
...
CameraTrackingGeoStatus.FIELDS = [
    ...
    new mavlink_1.MavLinkPacketField('camera_device_id', 'cameraDeviceId', 49, true, 1, 'uint8_t', ''),
];
/**
 * Camera absolute thermal range. This can be streamed when the associated VIDEO_STREAM_STATUS `flag`
 * field bit VIDEO_STREAM_STATUS_FLAGS_THERMAL_RANGE_ENABLED is set, but a GCS may choose to only
 * request it for the current active stream. Use MAV_CMD_SET_MESSAGE_INTERVAL to define message
 ...
 */
class CameraThermalRange extends mavlink_1.MavLinkData {
    constructor() {
        super();
        this.timeBootMs = 0;
        this.streamId = 0;
...
exports.CameraThermalRange = CameraThermalRange;
CameraThermalRange.MSG_ID = 277;
CameraThermalRange.MSG_NAME = 'CAMERA_THERMAL_RANGE';
CameraThermalRange.PAYLOAD_LENGTH = 30;
CameraThermalRange.MAGIC_NUMBER = 62;
CameraThermalRange.FIELDS = [
    new mavlink_1.MavLinkPacketField('time_boot_ms', 'timeBootMs', 0, false, 4, 'uint32_t', 'ms'),
    new mavlink_1.MavLinkPacketField('max', 'max', 4, false, 4, 'float', 'degC'),
    new mavlink_1.MavLinkPacketField('max_point_x', 'maxPointX', 8, false, 4, 'float', ''),
    new mavlink_1.MavLinkPacketField('max_point_y', 'maxPointY', 12, false, 4, 'float', ''),
    new mavlink_1.MavLinkPacketField('min', 'min', 16, false, 4, 'float', 'degC'),
    new mavlink_1.MavLinkPacketField('min_point_x', 'minPointX', 20, false, 4, 'float', ''),
    new mavlink_1.MavLinkPacketField('min_point_y', 'minPointY', 24, false, 4, 'float', ''),
    new mavlink_1.MavLinkPacketField('stream_id', 'streamId', 28, false, 1, 'uint8_t', ''),
    new mavlink_1.MavLinkPacketField('camera_device_id', 'cameraDeviceId', 29, false, 1, 'uint8_t', ''),
];
/**
...
AisVessel.FIELDS = [
    ...
    new mavlink_1.MavLinkPacketField('turn_rate', 'turnRate', 26, false, 1, 'int8_t', 'ddeg/s'),
 ...
 *
 * @deprecated since 2025-03, replaced by Nothing; Implemented PX4 v1.11 to v1.14. Not used in current flight stacks.
 */
 ...
 * stack, flight stack to GCS. Use BATTERY_STATUS for the frequent battery updates.
 *
 * @deprecated since 2024-02, replaced by BATTERY_INFO; The BATTERY_INFO message is better aligned with UAVCAN messages, and in any case is useful even if a battery is not "smart".
 */
...
/**
 * Fuel status. This message provides "generic" fuel level information for in a GCS and for triggering
 * failsafes in an autopilot. The fuel type and associated units for fields in this message are defined
 * in the enum MAV_FUEL_TYPE. The reported `consumed_fuel` and `remaining_fuel` must only be supplied
 * if measured: they must not be inferred from the `maximum_fuel` and the other value. A recipient can
 * assume that if these fields are supplied they are accurate. If not provided, the recipient can infer
 * `remaining_fuel` from `maximum_fuel` and `consumed_fuel` on the assumption that the fuel was
 * initially at its maximum (this is what battery monitors assume). Note however that this is an
 * assumption, and the UI should prompt the user appropriately (i.e. notify user that they should fill
 * the tank before boot). This kind of information may also be sent in fuel-specific messages such as
 * BATTERY_STATUS_V2. If both messages are sent for the same fuel system, the ids and corresponding
 * information must match. This should be streamed (nominally at 0.1 Hz).
 */
class FuelStatus extends mavlink_1.MavLinkData {
    constructor() {
        super();
        this.id = 0;
        this.maximumFuel = 0;
        ...
        this.flowRate = 0;
        this.temperature = 0;
        this.fuelType = MavFuelType[Object.keys(MavFuelType)[0]];
    }
}
exports.FuelStatus = FuelStatus;
FuelStatus.MSG_ID = 371;
FuelStatus.MSG_NAME = 'FUEL_STATUS';
FuelStatus.PAYLOAD_LENGTH = 26;
FuelStatus.MAGIC_NUMBER = 10;
FuelStatus.FIELDS = [
    new mavlink_1.MavLinkPacketField('maximum_fuel', 'maximumFuel', 0, false, 4, 'float', ''),
    new mavlink_1.MavLinkPacketField('consumed_fuel', 'consumedFuel', 4, false, 4, 'float', ''),
    new mavlink_1.MavLinkPacketField('remaining_fuel', 'remainingFuel', 8, false, 4, 'float', ''),
    new mavlink_1.MavLinkPacketField('flow_rate', 'flowRate', 12, false, 4, 'float', ''),
    new mavlink_1.MavLinkPacketField('temperature', 'temperature', 16, false, 4, 'float', 'K'),
    new mavlink_1.MavLinkPacketField('fuel_type', 'fuelType', 20, false, 4, 'uint32_t', ''),
    new mavlink_1.MavLinkPacketField('id', 'id', 24, false, 1, 'uint8_t', ''),
    new mavlink_1.MavLinkPacketField('percent_remaining', 'percentRemaining', 25, false, 1, 'uint8_t', '%'),
];
/**
...
class ComponentInformationBasic extends mavlink_1.MavLinkData {
    constructor() {
        super();
        this.timeBootMs = 0;
        this.capabilities = standard_1.MavProtocolCapability[Object.keys(standard_1.MavProtocolCapability)[0]];
        this.timeManufactureS = 0;
        this.vendorName = '';
        this.modelName = '';
...
ComponentInformationBasic.PAYLOAD_LENGTH = 160;
ComponentInformationBasic.MAGIC_NUMBER = 50;
ComponentInformationBasic.FIELDS = [
    new mavlink_1.MavLinkPacketField('capabilities', 'capabilities', 0, false, 8, 'uint64_t', ''),
    new mavlink_1.MavLinkPacketField('time_boot_ms', 'timeBootMs', 8, false, 4, 'uint32_t', 'ms'),
    new mavlink_1.MavLinkPacketField('time_manufacture_s', 'timeManufactureS', 12, false, 4, 'uint32_t', 's'),
    new mavlink_1.MavLinkPacketField('vendor_name', 'vendorName', 16, false, 1, 'char[]', '', 32),
    new mavlink_1.MavLinkPacketField('model_name', 'modelName', 48, false, 1, 'char[]', '', 32),
    new mavlink_1.MavLinkPacketField('software_version', 'softwareVersion', 80, false, 1, 'char[]', '', 24),
    new mavlink_1.MavLinkPacketField('hardware_version', 'hardwareVersion', 104, false, 1, 'char[]', '', 24),
    new mavlink_1.MavLinkPacketField('serial_number', 'serialNumber', 128, false, 1, 'char[]', '', 32),
];
/**
...
/**
 * Information about a flight mode. The message can be enumerated to get information for all modes, or
 * requested for a particular mode, using MAV_CMD_REQUEST_MESSAGE. Specify 0 in param2 to request that
 * the message is emitted for all available modes or the specific index for just one mode. The modes
 * must be available/settable for the current vehicle/frame type. Each mode should only be emitted once
 * (even if it is both standard and custom). Note that the current mode should be emitted in
 * CURRENT_MODE, and that if the mode list can change then AVAILABLE_MODES_MONITOR must be emitted on
 * first change and subsequently streamed. See https://mavlink.io/en/services/standard_modes.html
 */
 ...
 * (nominally 0.5 Hz). It may be requested using MAV_CMD_REQUEST_MESSAGE. See
 ...
 * the first change and should then be broadcast at low rate (nominally 0.3 Hz) and on change. See
 * https://mavlink.io/en/services/standard_modes.html
 */
 ...
 * Illuminator status
 */
class IlluminatorStatus extends mavlink_1.MavLinkData {
    constructor() {
        super();
        this.uptimeMs = 0;
        this.enable = 0;
        this.modeBitmask = IlluminatorMode[Object.keys(IlluminatorMode)[0]];
        this.errorStatus = IlluminatorErrorFlags[Object.keys(IlluminatorErrorFlags)[0]];
        this.mode = IlluminatorMode[Object.keys(IlluminatorMode)[0]];
        this.brightness = 0;
        this.strobePeriod = 0;
        this.strobeDutyCycle = 0;
        this.tempC = 0;
        this.minStrobePeriod = 0;
        this.maxStrobePeriod = 0;
    }
}
exports.IlluminatorStatus = IlluminatorStatus;
IlluminatorStatus.MSG_ID = 440;
IlluminatorStatus.MSG_NAME = 'ILLUMINATOR_STATUS';
IlluminatorStatus.PAYLOAD_LENGTH = 35;
IlluminatorStatus.MAGIC_NUMBER = 66;
IlluminatorStatus.FIELDS = [
    new mavlink_1.MavLinkPacketField('uptime_ms', 'uptimeMs', 0, false, 4, 'uint32_t', 'ms'),
    new mavlink_1.MavLinkPacketField('error_status', 'errorStatus', 4, false, 4, 'uint32_t', ''),
    new mavlink_1.MavLinkPacketField('brightness', 'brightness', 8, false, 4, 'float', '%'),
    new mavlink_1.MavLinkPacketField('strobe_period', 'strobePeriod', 12, false, 4, 'float', 's'),
    new mavlink_1.MavLinkPacketField('strobe_duty_cycle', 'strobeDutyCycle', 16, false, 4, 'float', '%'),
    new mavlink_1.MavLinkPacketField('temp_c', 'tempC', 20, false, 4, 'float', ''),
    new mavlink_1.MavLinkPacketField('min_strobe_period', 'minStrobePeriod', 24, false, 4, 'float', 's'),
    new mavlink_1.MavLinkPacketField('max_strobe_period', 'maxStrobePeriod', 28, false, 4, 'float', 's'),
    new mavlink_1.MavLinkPacketField('enable', 'enable', 32, false, 1, 'uint8_t', ''),
    new mavlink_1.MavLinkPacketField('mode_bitmask', 'modeBitmask', 33, false, 1, 'uint8_t', ''),
    new mavlink_1.MavLinkPacketField('mode', 'mode', 34, false, 1, 'uint8_t', ''),
];
/**
...
class OpenDroneIdBasicId extends mavlink_1.MavLinkData {
    constructor() {
        ...
        this.targetComponent = 0;
        this.idOrMac = [];
        this.idType = MavOdidIdType[Object.keys(MavOdidIdType)[0]];
        this.uaType = MavOdidUaType[Object.keys(MavOdidUaType)[0]];
        this.uasId = [];
    }
}
...
class OpenDroneIdLocation extends mavlink_1.MavLinkData {
    constructor() {
        ...
        this.targetComponent = 0;
        this.idOrMac = [];
        this.status = MavOdidStatus[Object.keys(MavOdidStatus)[0]];
        this.direction = 0;
        ...
        this.heightReference = MavOdidHeightRef[Object.keys(MavOdidHeightRef)[0]];
        this.height = 0;
        this.horizontalAccuracy = MavOdidHorAcc[Object.keys(MavOdidHorAcc)[0]];
        this.verticalAccuracy = MavOdidVerAcc[Object.keys(MavOdidVerAcc)[0]];
        this.barometerAccuracy = MavOdidVerAcc[Object.keys(MavOdidVerAcc)[0]];
        this.speedAccuracy = MavOdidSpeedAcc[Object.keys(MavOdidSpeedAcc)[0]];
        this.timestamp = 0;
        this.timestampAccuracy = MavOdidTimeAcc[Object.keys(MavOdidTimeAcc)[0]];
    }
}
...
class OpenDroneIdAuthentication extends mavlink_1.MavLinkData {
    constructor() {
        ...
        this.targetComponent = 0;
        this.idOrMac = [];
        this.authenticationType = MavOdidAuthType[Object.keys(MavOdidAuthType)[0]];
        this.dataPage = 0;
...
class OpenDroneIdSelfId extends mavlink_1.MavLinkData {
    constructor() {
        ...
        this.targetComponent = 0;
        this.idOrMac = [];
        this.descriptionType = MavOdidDescType[Object.keys(MavOdidDescType)[0]];
        this.description = '';
    }
}
...
class OpenDroneIdSystem extends mavlink_1.MavLinkData {
    constructor() {
        ...
        this.targetComponent = 0;
        this.idOrMac = [];
        this.operatorLocationType = MavOdidOperatorLocationType[Object.keys(MavOdidOperatorLocationType)[0]];
        this.classificationType = MavOdidClassificationType[Object.keys(MavOdidClassificationType)[0]];
        ...
        this.areaCeiling = 0;
        this.areaFloor = 0;
        this.categoryEu = MavOdidCategoryEu[Object.keys(MavOdidCategoryEu)[0]];
        this.classEu = MavOdidClassEu[Object.keys(MavOdidClassEu)[0]];
...
class OpenDroneIdOperatorId extends mavlink_1.MavLinkData {
    constructor() {
        ...
        this.targetComponent = 0;
        this.idOrMac = [];
        this.operatorIdType = MavOdidOperatorIdType[Object.keys(MavOdidOperatorIdType)[0]];
        this.operatorId = '';
    }
}
...
class NavLoiterTimeCommand extends CommandLong {
    ...
    /**
     * Leave loiter circle only when track heading towards the next waypoint (MAV_BOOL_FALSE: Leave on time
     * expiry). Values not equal to 0 or 1 are invalid.
     */
    get headingRequired() {
        return this._param2;
    }
...
class NavLoiterToAltCommand extends CommandLong {
    ...
    /**
     * Leave loiter circle only when track heading towards the next waypoint (MAV_BOOL_FALSE: Leave when
     * altitude reached). Values not equal to 0 or 1 are invalid.
     */
    get headingRequired() {
        return this._param1;
    }
...
class DoSetModeCommand extends CommandLong {
     ...
     * MAV_MODE_FLAG_CUSTOM_MODE_ENABLED is set in param1 (mode) this mode is used: otherwise the field is
     * ignored.
     */
    get customMode() {
        return this._param2;
...
class DoSetHomeCommand extends CommandLong {
    ...
    /**
     * Roll angle (of surface). Range: -180..180 degrees. NAN or 0 means value not set. 0.01 indicates zero
     * roll.
     *
     * @units deg
     * @min: -180
 ...
 * This command has location.
 */
class DoReturnPathStartCommand extends CommandLong {
    constructor(targetSystem = 1, targetComponent = 1) {
        super();
        this.command = MavCmd.DO_RETURN_PATH_START;
        this.targetSystem = targetSystem;
        this.targetComponent = targetComponent;
    }
    /**
...
exports.DoReturnPathStartCommand = DoReturnPathStartCommand;
/**
 * Mission item to mark the start of a mission landing pattern, or a command to land with a mission
 * landing pattern. When used in a mission, this is a marker for the start of a sequence of mission
 * items that represent a landing pattern. It should be followed by a navigation item that defines the
...
class DoRepositionCommand extends CommandLong {
    ...
    /**
     * Yaw heading (heading reference defined in Bitmask field). NaN to use the current system yaw heading
     * mode (e.g. yaw towards next waypoint, yaw to home, etc.). For planes indicates loiter direction (0:
...
class DoPauseContinueCommand extends CommandLong {
    ...
    /**
     * Continue mission (MAV_BOOL_TRUE), Pause current mission or reposition command, hold current position
     * (MAV_BOOL_FALSE). Values not equal to 0 or 1 are invalid. A VTOL capable vehicle should enter hover
     * mode (multicopter and VTOL planes). A plane should loiter with the default loiter radius.
     */
    get continue() {
        return this._param1;
    }
...
class DoFenceEnableCommand extends CommandLong {
    ...
    /**
     * Fence types to enable or disable as a bitmask. 0: field is unused/all fences should be enabled or
     * disabled (for compatibility reasons). Parameter is ignored if param1=2.
     */
    get types() {
        return this._param2;
    }
...
class DoEngineControlCommand extends CommandLong {
    ...
    /**
     * Cold start engine (MAV_BOOL_FALSE: Warm start). Values not equal to 0 or 1 are invalid. Controls use
     * of choke where applicable
     */
    get coldStart() {
        return this._param2;
    }
 ...
 * should ACK with MAV_RESULT_FAILED. See https://mavlink.io/en/services/standard_modes.html
 */
class DoSetStandardModeCommand extends CommandLong {
    constructor(targetSystem = 1, targetComponent = 1) {
        super();
 ...
 * illuminating the system itself, e.g. an indicator light).
 */
class DoIlluminatorConfigureCommand extends CommandLong {
    constructor(targetSystem = 1, targetComponent = 1) {
        super();
        this.command = MavCmd.DO_ILLUMINATOR_CONFIGURE;
        this.targetSystem = targetSystem;
        this.targetComponent = targetComponent;
    }
    /**
    ...
    set strobeDuty(value) {
        this._param4 = value;
    }
}
exports.DoIlluminatorConfigureCommand = DoIlluminatorConfigureCommand;
/**
 * Request the home position from the vehicle. 	 The vehicle will ACK the command and then emit the
 * HOME_POSITION message.
 */
...
class SetMessageIntervalCommand extends CommandLong {
     ...
     * requested message. By default assumed not used (0). When used as an index ID, 0 means "all
     ...
     * (recommended), 1: address of requester, 2: broadcast.
     *
     * @min: 0
     * @max: 2
     * @increment: 1
...
class RequestProtocolVersionCommand extends CommandLong {
    ...
    /**
     * Request supported protocol versions by all nodes on the network (MAV_BOOL_TRUE). Values not equal to
     * 0 or 1 are invalid.
     */
    get protocol() {
        return this._param1;
...
class StorageFormatCommand extends CommandLong {
     ...
     * CAMERA_CAPTURE_STATUS.image_count and CAMERA_IMAGE_CAPTURED.image_index. Values not equal to 0 or 1
     * are invalid.
     */
    get resetImageLog() {
        return this._param3;
 ...
 * Set camera source. Changes the camera's active sources on cameras with multiple image sensors.
 */
class SetCameraSourceCommand extends CommandLong {
    constructor(targetSystem = 1, targetComponent = 1) {
        super();
        this.command = MavCmd.SET_CAMERA_SOURCE;
        this.targetSystem = targetSystem;
        this.targetComponent = targetComponent;
    }
    /**
    ...
    set secondarySource(value) {
        this._param3 = value;
    }
}
exports.SetCameraSourceCommand = SetCameraSourceCommand;
/**
 * Tagged jump target. Can be jumped to with MAV_CMD_DO_JUMP_TAG.
 */
...
class ControlHighLatencyCommand extends CommandLong {
    ...
    /**
     * Start transmission over high latency telemetry (MAV_BOOL_FALSE: stop transmission). Values not equal
     * to 0 or 1 are invalid.
     */
    get enable() {
        return this._param1;
    }
...
class NavFencePolygonVertexInclusionCommand extends CommandLong {
    ...
    /**
     * Polygon vertex count. This is the number of vertices in the current polygon (all vertices will have
     * the same number).
     *
     * @min: 3
     * @increment: 1
 ...
 * This command has location.
 */
class ExternalPositionEstimateCommand extends CommandLong {
    constructor(targetSystem = 1, targetComponent = 1) {
        super();
...
exports.COMMANDS = {
    ...
    [MavCmd.DO_RETURN_PATH_START]: DoReturnPathStartCommand,
    ...
    [MavCmd.DO_ILLUMINATOR_CONFIGURE]: DoIlluminatorConfigureCommand,
    ...
    [MavCmd.SET_CAMERA_SOURCE]: SetCameraSourceCommand,
